...
#' candidate retrieval from the curated marker resource, the annotating agent and
#' the evidence review that checks it, and the delivered result package (tables,
#' audit sidecar, figures, offline viewer and ZIP).
#'
...
#' @param tissue Free-text tissue, resolved against UBERON. Several names joined by
#'   `"|"` (e.g. `"urinary bladder|adipose tissue"`) are one operator-fixed list: the
#'   marker menu is the union of each listed tissue's own UBERON closure, every
#'   retrieved candidate records which listed tissue admitted it, and the list is
#'   written verbatim to the run manifest. A single name is the default behaviour.
