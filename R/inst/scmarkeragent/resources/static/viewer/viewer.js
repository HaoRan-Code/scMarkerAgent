...
const PHRASES = {
  ...
  annotation_qc: {
    passed: "Held up against the literature",
    passed_after_revision: "Revised, then held up",
    arbitrated: "Rounds disagreed — decided from the evidence",
    failed: "Review did not carry it — last answer shown",
...
function showDetail(row, scroll = false) {
  ...
  addDefinition(definitions, "Evidence review", phrase("annotation_qc", row.annotation_qc));
  addDefinition(definitions, "Review rounds", format(row.evidence_review_rounds));
  addDefinition(definitions, "Delivered from tier", format(row.delivered_tier));
  ...
  const rounds = record?.audit?.evidence_review?.rounds || [];
  if (rounds.length) {
    rationale.append(make("h3", "", "Evidence review"));
    rounds.forEach(round => {
      const verdict = round.verdict || {};
      const finer = verdict.subtype_verdict ? `, finer name ${verdict.subtype_verdict}` : "";
      const better = verdict.better_candidate ? ` -> ${verdict.better_candidate}` : "";
      rationale.append(make("p", "", `Round ${round.round} (tier ${round.tier}) ${round.selected}`
        + `${round.subtype ? ` / ${round.subtype}` : ""}: ${verdict.identity_verdict || "not checked"}`
        + `${finer}${better}. ${verdict.reason || ""}`));
    });
  }
  const arbitration = record?.audit?.arbitration;
  if (arbitration && arbitration.selected) {
    rationale.append(make("h3", "", "Arbitration"));
    rationale.append(make("p", "", `Delivered ${arbitration.selected}`
      + `${arbitration.subtype ? ` / ${arbitration.subtype}` : ""}. ${arbitration.reason || ""}`));
  }
...
function renderClusters() {
  ...
  filtered.forEach(row => {
    ...
    if (Number(row.delivered_tier) > 1) {
      typeCell.append(make("div", "sub", `first ${(Number(row.delivered_tier) - 1) * 15} candidates did not carry it`));
    }
    tr.append(typeCell);
