...
.llm_cache_lock_path <- function(path) paste0(path, ".lock")
.llm_pid_alive <- function(pid) {
  ...
  isTRUE(tryCatch(tools::pskill(pid, signal = 0L), error = function(e) NA))
}
.llm_acquire_cache_lock <- function(path) {
  lock <- .llm_cache_lock_path(path)
  mine <- Sys.getpid()
  if (file.exists(lock)) {
    holder <- suppressWarnings(as.integer(trimws(readLines(lock, n = 1L, warn = FALSE)[1])))
    if (length(holder) && !is.na(holder) && holder != mine && .llm_pid_alive(holder)) {
      stop(sprintf(
        paste0(
          "LLM response cache is already held by live process %d: %s\n",
          "Two annotation processes sharing one cache interleave the response cache and ",
          "the cold-call log between them, and neither process's trace can be trusted ",
          "afterwards. Stop process %d first, or point SCMA_LLM_CACHE at a fresh path ",
          "for this run."
        ),
        holder, path, holder
      ), call. = FALSE)
  ...
  writeLines(as.character(mine), lock)
  previous_locks <- if (is.null(.LLM_CACHE$locks)) character(0) else .LLM_CACHE$locks
  .LLM_CACHE$locks <- union(previous_locks, lock)
  if (!isTRUE(.LLM_CACHE$lock_finalizer_registered)) {
    .LLM_CACHE$lock_finalizer_registered <- TRUE
    reg.finalizer(.LLM_CACHE, function(env) {
      held_locks <- if (is.null(env$locks)) character(0) else env$locks
      for (held in held_locks) {
        owner <- suppressWarnings(as.integer(trimws(readLines(held, n = 1L, warn = FALSE)[1])))
        if (length(owner) && !is.na(owner) && owner == mine) {
          unlink(held)
        }
      }
    }, onexit = TRUE)
