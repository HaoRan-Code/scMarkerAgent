...
# and matched the shared server method for method, while `cluster_annotation.R` still
...
## Packaged adaptation (departure one of two from software/tests/test_r_source_stream.R):
...
## the arm instead and requires the exemption in both loops: it is a one-line deletion
## that turns "these three did not settle it" back into an unanswerable request, and no
## other test in this suite would notice.
## Packaged adaptation (departure two of two): the upstream test asserts the same
## exemption in the other arm's cluster annotation loop, whose sources do not ship in
## this package; that half runs in the monorepo's own test suite.
loop_r <- paste(
  readLines(file.path(package, "rflow", "cluster_annotation.R"), warn = FALSE),
  collapse = "\n"
)
stopifnot(
  ## The annotator loop and the judge loop, on this arm.
