...
## Packaged adaptation (the one departure from software/tests/test_r_llm_client.R):
...
## failed assertion.
## Packaged adaptation: R CMD check --as-cran interposes a decoy `Rscript` on PATH
## that refuses to run ("'Rscript' should not be used without a path"), so the
## provider is started through R.home() as CRAN policy asks.
system2(
  file.path(R.home("bin"), "Rscript"), c(server_script, port_file),
  wait = FALSE,
...
## Packaged adaptation: on startup failure, surface the provider's own log -- the
## child session's temp directory dies with it, and this message is then the only
## record of why the server never came up.
if (!file.exists(port_file)) {
  provider_log <- file.path(work, "fake_provider.log")
  if (file.exists(provider_log)) {
    message("fake provider log:\n", paste(readLines(provider_log, warn = FALSE), collapse = "\n"))
  } else {
    message("fake provider log was never created")
  }
  stop("fake provider did not start")
}
