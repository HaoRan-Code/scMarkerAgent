...
differential-expression engine that is not on CRAN. One `install.packages()` brings
in the required packages, with prebuilt binaries on Windows and macOS:
...
The pipeline itself, with the shared configuration, prompts, schemas and viewer assets,
ships under `inst/scmarkeragent/` and runs in a fresh `Rscript` process.
`DESCRIPTION` lists the required dependencies. For a pinned dependency environment,
use `inst/environment/renv.lock`; set `SCMA_PRESTO_SOURCE` to a local checkout of the
`presto` revision shown above, then run:
...
| `tissue` | free text, resolved against UBERON; several names joined by `\|` (e.g. `urinary bladder\|adipose tissue`) are one operator list: the marker menu is the union of each tissue's UBERON closure, every candidate records which listed tissue admitted it, and the list is written to the run manifest |
...
| `cluster_evidence.jsonl` | audit sidecar: every retrieved candidate with its scores and complete measured panel, the agent's turns and tool calls, the exclusion and definer audits, the review verdicts and any arbitration, and the dataset-wide reading of the cluster's name |
