...
def _parser():
    ...
    annotate = sub.add_parser(
        ...
        help=(
            ...
            "the annotating agent and the evidence review that checks it, and the delivered result package"
        ),
    )
    ...
    annotate.add_argument(
        ...
        required=True,
        help=(
            "tissue context of the dataset. Several names joined by '|' (e.g. "
            "'urinary bladder|adipose tissue') are one operator-fixed LIST: the marker "
            "menu is the union of each listed tissue's own UBERON self+descendant closure, "
            "every retrieved candidate records which listed tissue admitted it, and the "
            "list is written verbatim to the run manifest. A single name is the default "
...
def _annotate(args):
    ...
    from .marker_database import parse_tissue_arg, tissue_list
    try:
        tissue = parse_tissue_arg(args.tissue)
