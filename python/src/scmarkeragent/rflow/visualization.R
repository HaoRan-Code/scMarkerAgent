...
if (!exists(".cp", mode = "function")) {
  source(file.path(.VIS_SELF, "config.R"))
}
...
.vis_umap <- function(cells, categories, color_of, title, stem,
  ...
  if (is.null(order)) {
    order <- sort(.cp(unique(as.character(categories))), method = "radix")
  }
  .vis_umap_panel(
...
#' Render reference-free figures for one completed run, reading only the
#' figure_data bundle.
