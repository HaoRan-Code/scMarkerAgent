...
cfg <- list(
  obj = INPUT_RDS,
  species = INPUT_SPECIES, tissue = INPUT_TISSUE_CONTEXTS, disease = INPUT_DISEASE,
...
cat(sprintf(
  ...
  tag, cfg$species, paste(cfg$tissue, collapse = "|"), paste(cfg$disease, collapse = "+"),
...
if (anyDuplicated(.key)) {
  ...
  rownames(counts) <- .sym[match(.ukey, .key)]
  cat(sprintf(
...
mito_feats <- rownames(o)[grepl("^MT-", toupper(rownames(o)))]
...
o <- ScaleData(o, verbose = FALSE, scale.max = as.numeric(CFG$preprocessing$scale_max_value))
o <- RunPCA(
  o,
...
if (length(unique(clusters)) < 2L) {
  stop(sprintf(paste0(
    ...
    "differential expression is undefined for a single cluster. Raise clustering_resolution, ",
    "or check that the input holds more than one population."
  ), RES), call. = FALSE)
}
...
.menu_measured_genes <- function(measured, sp_q, ti_q, di_q) {
  ...
  tissue_closure <- function(ti, tissue_root = TISSUE_ROOT) {
    scma_tissue_closure(UB, ti, tissue_root)
  }
  db <- scma_load_marker_db(DB_CSV)
  setDT(db)
  .ctl <- tolower(db$cell_type)
  .mm <- rep(FALSE, nrow(db))
  for (.sp in names(XSP)) {
  ...
  if (!length(positive_genes)) {
    ...
    stop(sprintf(
      ...
      length(unique(as.character(menu_pos))), sp_q, paste(ti_q, collapse = "|"), paste(di_q, collapse = "+"),
...
mat <- GetAssayData(o, assay = "RNA", layer = "data")
